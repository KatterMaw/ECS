...
public sealed class TwoComponentsSystemWithOptionalTests
{
			...
			public void Update()
			{
				for (var archetypeIndex = 0; archetypeIndex < _query.Archetypes.Count; archetypeIndex++)
				{
					...
					for (int i = 0; i < archetype.EntitiesCount; i++)
					{
						...
						Velocity? velocityDefaultValue = null;
						ref Velocity? velocity = ref velocityDefaultValue;
						if (velocitySpan.Length != 0)
						{
							ref Velocity velocityTemp = ref velocitySpan[i];
							velocity = velocityTemp;
						}
						Update(ref position, ref velocity);
					}
				}
			}
	...
	private const string GeneratorInput =
		...
		namespace Tests.Sample.Systems
		{
			...
			{
				private void Update(ref Position position, ref Velocity? velocity)
				{
				}
			}
		}
